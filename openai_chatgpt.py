...
__date__ = "2022-03-31"
__env__ = '.env'
__package_name__ = 'esp-gpt'

ESPANSO_PACKAGES_DIR = '/match/packages'
ESPANSO_PATH = os.popen('echo "$(espanso path config)"').read().rstrip()
PACKAGE_PATH = f'{ESPANSO_PATH}{ESPANSO_PACKAGES_DIR}/{__package_name__}'
...
config['system_role'] = os.getenv('system_role')
config['address'] = os.getenv('address')
config['conversation_timeout_minutes'] = os.getenv('conversation_timeout_minutes')
config['allow_clipboard'] = os.getenv('allow_clipboard')
config['transcription_model'] = os.getenv('transcription_model')
config['record_duration'] = os.getenv('record_duration')
config['record'] = os.getenv('record')

config['recording_path'] = f'{PACKAGE_PATH}/prompt.wav'
config['sound_effect'] = f'{PACKAGE_PATH}/bell.wav'
config['db_file'] = f'{PACKAGE_PATH}/chats.db'
...
def send_request(data: dict, address: str, api_key: str) -> dict:
    """
    Send a request to the openAI API endpoint.

    Args:
        data (dict): The dict data to send.
    ...
    logger.info(f"Response received from the AI model: {response}")
...
def run_chatgpt(
    ...
    # Create json request
    timestamp_cutoff = timestamp_n_minutes_ago(int(conversation_timeout_minutes))
    messages = (
    ...
    data = {"model": model, "temperature": float(temperature), "messages": messages}
    #json_data = json.dumps(data)
    # Get the reply from the AI model
    response = send_request(data, address, openai_api_key)
...
def parse_args():
    ...
    parser.add_argument(
        "--sound-effect",
        default=None,
        help="Path to the sound effect .waClipboard content were cleared."
    )
